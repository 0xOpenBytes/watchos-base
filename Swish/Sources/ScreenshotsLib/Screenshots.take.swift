  ...
  func take() throws {
    ...
    try Device.allCases.forEach { device in
      try sh(.file(logsPath + "/\(device.name).build.log"), """
        xcodebuild -project base.xcodeproj -scheme Screenshots -destination 'platform=watchOS Simulator,name=\(device.simulatorName)' -derivedDataPath "\(derivedDataPath)" test
        """)
